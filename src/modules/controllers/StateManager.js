...
export class StateManager {
  ...
  #hydrate() {
    this.projects = Object.fromEntries(
      Object.entries(this.projects).map((project) => {
        ...
        hydratedProject.activeTasks = project[1].activeTasks.map(
          (task) => new Task(task),
        );
        hydratedProject.completedTasks = project[1].completedTasks.map(
          (task) => new Task(task),
        );
  ...
  editTask = (taskData, currentProjectId, taskId) => {
    const newTask = new Task(taskData);
    const taskArray =
      this.projects[currentProjectId][this.uiState.currentTasksArr];
